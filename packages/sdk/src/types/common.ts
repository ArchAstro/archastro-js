...
export const billingSettingsOrgSchema = z.object({
  auto_reup_amount_cents: z.number().int().optional(), // Amount (cents) to charge when threshold is crossed
  auto_reup_enabled: z.boolean().optional(), // Whether automatic credit reup is enabled
  auto_reup_threshold_cents: z.number().int().optional(), // Balance threshold (cents) to trigger auto-reup
  ...
  pending_plan: z.string().optional(), // Plan change awaiting Stripe confirmation, or __clear__ to remove plan
  plan: z.string().optional(), // Active billing plan (e.g. enterprise-pilot)
...
export const checkoutSessionResultSchema = z.object({
  checkout_url: z.string(), // Stripe Checkout URL to redirect the user to
});
...
export const creditBalanceSchema = z.object({
  available_cents: z.number().int(), // Available credit balance in cents
  currency: z.string(), // Currency code (e.g. usd)
  has_payment_method: z.boolean(), // Whether a saved payment method exists
});
...
export const developerSchema = z.object({
  ...
  billing_provider_id: z.string().optional(), // Stripe customer ID
  pending_plan: z.string().optional(), // Plan change awaiting Stripe confirmation, or __clear__ to remove plan
  plan: z.string().optional(), // Active billing plan (e.g. paygo)
});
...
export const keyValueStorageEntryListSchema = z.object({
  data: z.array(keyValueStorageEntrySchema), // Storage entries owned by the caller
});
export type KeyValueStorageEntryList = z.infer<typeof keyValueStorageEntryListSchema>;
