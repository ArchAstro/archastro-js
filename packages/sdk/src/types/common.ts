...
export const attachmentSchema = z.object({
  ...
  /** The full embedded object payload. For `task` type, contains the task record. For `action` type, contains the action definition. For `chart` type, contains the chart with its inline `spec`. Omitted on other types. */
  object: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("The full embedded object payload. For `task` type, contains the task record. For `action` type, contains the action definition. For `chart` type, contains the chart with its inline `spec`. Omitted on other types."),
  /** Display title. The page title for `scraped_link`, the artifact name for `artifact`, and the task title for `task` types. `null` on other types. */
...
}).describe("A failed local-tool outcome.") satisfies z.ZodType<Error>;
...
}).describe("Billing administration authority for one Viewer-derived principal kind.") satisfies z.ZodType<EffectiveAccessBillingPrincipal>;
...
}).describe("Live effective access and billing authority for the authenticated user.") satisfies z.ZodType<EffectiveAccess>;
