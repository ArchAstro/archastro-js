...
export interface Thread {
  ...
  visibility: "team" | "restricted" | "private" | "org";
}
...
export const threadSchema = z.object({
  ...
  /** Thread subtype: `"standard"` for ordinary threads, `"personal"` for a user-and-owned-agents roster, `"feed"` for a post feed with inline replies, `"slack_mirror"` for the membership-strict mirror of a Slack channel, or `"slashwork_mirror"` for the membership-strict mirror of a Slashwork group. `personal` and `feed` are explicit creation options; mirror kinds are server-derived. */
  kind: z.string().nullable().optional().describe("Thread subtype: `\"standard\"` for ordinary threads, `\"personal\"` for a user-and-owned-agents roster, `\"feed\"` for a post feed with inline replies, `\"slack_mirror\"` for the membership-strict mirror of a Slack channel, or `\"slashwork_mirror\"` for the membership-strict mirror of a Slashwork group. `personal` and `feed` are explicit creation options; mirror kinds are server-derived."),
  /** When the most recent message was posted in this thread, falling back to the thread's creation time if it has no messages. Always populated on thread list endpoints (which order by it, after default threads); `null` on endpoints that don't compute activity enrichment. */
  ...
  /** Arbitrary key-value metadata attached to the thread. Shape is application-defined; `null` if no metadata has been set. */
  metadata: z.record(z.custom<{} | null>((value) => value !== undefined)).nullable().optional().describe("Arbitrary key-value metadata attached to the thread. Shape is application-defined; `null` if no metadata has been set."),
  /** Whether the authenticated user has muted notifications for this thread. `true` suppresses all notification delivery. */
  ...
  /** Threads that are nested under this thread as replies to a parent message. Present only when sub-thread enrichment is requested. */
  sub_threads: z.array(z.record(z.custom<{} | null>((value) => value !== undefined))).nullable().optional().describe("Threads that are nested under this thread as replies to a parent message. Present only when sub-thread enrichment is requested."),
  /** Status tags on the thread (e.g. `"blocked"`, `"needs-review"`). Edited by any thread participant via the `/threads/:thread/tags` endpoints and filterable on the thread list endpoints. Empty array if none set. */
  ...
  /** Who can read the thread: `team` for every owning-team member, `restricted` for team-readable threads with an explicit roster, `private` for roster-only access, or `org` for every member of the owning organization. */
  visibility: z.enum(["team", "restricted", "private", "org"]).describe("Who can read the thread: `team` for every owning-team member, `restricted` for team-readable threads with an explicit roster, `private` for roster-only access, or `org` for every member of the owning organization."),
...
export const threadMessageSchema = z.object({
  ...
  /** Admin-only diagnostic metadata for the message, including execution trajectory details. Only present in developer portal responses. */
  admin: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("Admin-only diagnostic metadata for the message, including execution trajectory details. Only present in developer portal responses."),
  /** Agent ID (`agt_...`) associated with the message. `null` if not agent-scoped. */
  ...
  /** Key-value metadata attached to the message. Always present; defaults to an empty object. The `metadata` query parameter filters on this same object, so a caller can read back the field it selects on. */
  metadata: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("Key-value metadata attached to the message. Always present; defaults to an empty object. The `metadata` query parameter filters on this same object, so a caller can read back the field it selects on."),
  /** Organization ID (`org_...`) this message is scoped to. `null` if not org-scoped. */
