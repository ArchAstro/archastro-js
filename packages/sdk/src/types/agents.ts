...
export const agentHealthSchema = z.object({
  activity: z.record(z.unknown()), // Last and next relevant activity timestamps
  agent: agentSchema, // Agent this health profile describes
  checked_at: z.string(), // Timestamp when the profile was computed
  checks: z.array(z.record(z.unknown())), // Renderable health checks with key, label, status, and summary
  counts: z.record(z.unknown()), // Counts grouped by dependency area and status
  recent: z.record(z.unknown()), // Recent activity and failure counts
  score: z.number().int(), // Normalized health score from 0 to 100
  status: z.string(), // Overall health status: ok, warning, or critical
});
