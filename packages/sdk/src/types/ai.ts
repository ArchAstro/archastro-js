...
export const aIChatStreamDoneSchema = z.object({
  ...
  /** Aggregate token usage across every run in the completion (including tool-call continuations), keyed by model ID. */
  total_usage: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("Aggregate token usage across every run in the completion (including tool-call continuations), keyed by model ID."),
  /** Token usage for the final run, keyed by model ID. */
  usage: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("Token usage for the final run, keyed by model ID."),
...
export const aIToolCallSchema = z.object({
  /** Arguments the model wants to pass to the tool, as a key-value map. Deserialize and validate these against the tool's input schema before executing. */
  arguments: z.record(z.custom<{} | null>((value) => value !== undefined)).describe("Arguments the model wants to pass to the tool, as a key-value map. Deserialize and validate these against the tool's input schema before executing."),
  /** Unique identifier for this tool call, assigned by the model. Use this value as `id` when submitting the corresponding tool result. */
...
export const aIToolResultSchema = z.object({
  ...
  /** Structured result data from the tool execution. Shape varies by tool. `null` when the result is expressed as plain text in `content`. */
  resolution: z.custom<{} | null>((value) => value !== undefined).optional().describe("Structured result data from the tool execution. Shape varies by tool. `null` when the result is expressed as plain text in `content`."),
...
export const aIMessageSchema = z.object({
  ...
  /** Multimodal content parts for the message, used when the body includes images or mixed media. Each part is a map with a `type` key (`"text"`, `"image_url"`, or `"image_data"`). `null` when `content` is set. */
  content_parts: z.array(z.record(z.custom<{} | null>((value) => value !== undefined))).optional().describe("Multimodal content parts for the message, used when the body includes images or mixed media. Each part is a map with a `type` key (`\"text\"`, `\"image_url\"`, or `\"image_data\"`). `null` when `content` is set."),
  /** Opaque token that can be passed on a subsequent request to resume this conversation from the current state. `null` when the provider does not support conversation resumption. */
  ...
  /** Parsed structured data returned by the model when a JSON schema or structured-output mode was requested. Shape varies by the schema supplied at call time. `null` when structured output was not requested. */
  structured_output: z.custom<{} | null>((value) => value !== undefined).optional().describe("Parsed structured data returned by the model when a JSON schema or structured-output mode was requested. Shape varies by the schema supplied at call time. `null` when structured output was not requested."),
  /** Tool calls requested by the model in an `assistant` message. Present only on assistant messages that invoke one or more tools. `null` on all other message roles. */
...
export const aIChatStreamMessageCompleteSchema = z.object({
  ...
  /** Token consumption for this run, keyed by model ID. `null` when usage data is unavailable. */
  usage: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("Token consumption for this run, keyed by model ID. `null` when usage data is unavailable."),
...
export interface AIChatStreamThinkingDelta {
  /** The chunk of model reasoning produced since the previous `thinking_delta` event. */
  delta: string;
}

/** Runtime validator for {@link AIChatStreamThinkingDelta}. */
export const aIChatStreamThinkingDeltaSchema = z.object({
  /** The chunk of model reasoning produced since the previous `thinking_delta` event. */
  delta: z.string().describe("The chunk of model reasoning produced since the previous `thinking_delta` event."),
}).describe("Incremental model reasoning emitted during a streaming chat completion (SSE `thinking_delta` event).") satisfies z.ZodType<AIChatStreamThinkingDelta>;
...
export const aICompletionResultSchema = z.object({
  ...
  /** Token consumption breakdown keyed by model ID. Each value is a map with `"input_tokens"` and `"output_tokens"` counts. `null` when usage data is unavailable. */
  token_usage: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("Token consumption breakdown keyed by model ID. Each value is a map with `\"input_tokens\"` and `\"output_tokens\"` counts. `null` when usage data is unavailable."),
...
export interface AIEvaluationAnswerBoolean {
  id: string;
  probability: number;
  type: "boolean";
}

/** Runtime validator for {@link AIEvaluationAnswerBoolean}. */
export const aIEvaluationAnswerBooleanSchema = z.object({
  id: z.string(),
  probability: z.number(),
  type: z.literal("boolean"),
}).describe("Probability that the statement is true, in `[0, 1]`.") satisfies z.ZodType<AIEvaluationAnswerBoolean>;

/** Selected option plus the full probability distribution. */
export interface AIEvaluationAnswerChoice {
  choice: string;
...
/** Runtime validator for {@link AIEvaluationAnswerChoice}. */
export const aIEvaluationAnswerChoiceSchema = z.object({
  choice: z.string(),
  confidence: z.number().optional(),
  id: z.string(),
  probabilities: z.record(z.custom<{} | null>((value) => value !== undefined)),
  type: z.literal("choice"),
}).describe("Selected option plus the full probability distribution.") satisfies z.ZodType<AIEvaluationAnswerChoice>;

/** Probability-weighted position along the caller-defined levels. */
export interface AIEvaluationAnswerScore {
  confidence?: number | undefined;
  id: string;
...
/** Runtime validator for {@link AIEvaluationAnswerScore}. */
export const aIEvaluationAnswerScoreSchema = z.object({
  confidence: z.number().optional(),
  id: z.string(),
  legend: z.record(z.custom<{} | null>((value) => value !== undefined)),
  probabilities: z.record(z.custom<{} | null>((value) => value !== undefined)),
  score: z.number(),
  type: z.literal("score"),
}).describe("Probability-weighted position along the caller-defined levels.") satisfies z.ZodType<AIEvaluationAnswerScore>;

/**
 * One typed evaluation answer. `type` selects boolean, choice, or score.
 * `id` matches the question id.
 */
export const aIEvaluationAnswerSchema = z.discriminatedUnion("type", [aIEvaluationAnswerBooleanSchema, aIEvaluationAnswerChoiceSchema, aIEvaluationAnswerScoreSchema]);
/**
 * One typed evaluation answer. `type` selects boolean, choice, or score.
 * `id` matches the question id.
 */
export type AIEvaluationAnswer = z.infer<typeof aIEvaluationAnswerSchema>;

/** Token counts for one evaluation request. */
export interface AIEvaluationUsage {
...
/** Runtime validator for {@link AIEvaluationUsage}. */
export const aIEvaluationUsageSchema = z.object({
  /** Provider-reported input tokens. `null` when the provider omitted the count. */
  input_tokens: z.number().int().optional().describe("Provider-reported input tokens. `null` when the provider omitted the count."),
  /** Provider-reported output tokens. `null` when the provider omitted the count. */
  output_tokens: z.number().int().optional().describe("Provider-reported output tokens. `null` when the provider omitted the count."),
}).describe("Token counts for one evaluation request.") satisfies z.ZodType<AIEvaluationUsage>;
...
export interface AIEvaluationResult {
  ...
  usage?: AIEvaluationUsage | undefined;
}

/** Runtime validator for {@link AIEvaluationResult}. */
export const aIEvaluationResultSchema = z.object({
  /** Typed answers. Each value is a boolean, choice, or score object with the question `id`. */
  answers: z.array(aIEvaluationAnswerSchema).describe("Typed answers. Each value is a boolean, choice, or score object with the question `id`."),
  /** Model identifier that produced the answers. */
  model: z.string().describe("Model identifier that produced the answers."),
  /** UUID grouping this evaluation's usage record. Generated when omitted on the request. */
  session_id: z.string().optional().describe("UUID grouping this evaluation's usage record. Generated when omitted on the request."),
  /** Token usage for this request. `null` when usage data is unavailable. */
  usage: aIEvaluationUsageSchema.optional().describe("Token usage for this request. `null` when usage data is unavailable."),
}).describe("Result of an evaluation request: typed answers, the model that ran, and token usage.") satisfies z.ZodType<AIEvaluationResult>;
...
export interface AIImageResult {
  ...
  usage_attempt_count?: number | undefined;
...
export const aIImageResultSchema = z.object({
  ...
  /** UUID grouping this image request's provider attempts for usage and billing. */
  session_id: z.string().optional().describe("UUID grouping this image request's provider attempts for usage and billing."),
  /** Canonical size string as returned by the provider, e.g. `"1024x1024"`. `null` when not reported. */
  size: z.string().optional().describe("Canonical size string as returned by the provider, e.g. `\"1024x1024\"`. `null` when not reported."),
  /** Provider-reported token and compute usage for the request. Structure varies by provider. `null` when usage data is unavailable. */
  usage: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("Provider-reported token and compute usage for the request. Structure varies by provider. `null` when usage data is unavailable."),
  /** Number of provider responses contributing usage records to this request, including image-generation retries. */
  usage_attempt_count: z.number().int().optional().describe("Number of provider responses contributing usage records to this request, including image-generation retries."),
  /** Width of the generated image in pixels. `null` when the provider does not report dimensions. */
