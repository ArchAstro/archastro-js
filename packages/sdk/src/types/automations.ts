// Copyright (c) 2026 ArchAstro Inc. Licensed under the MIT License.
// This file is auto-generated by @archastro/sdk-generator. Do not edit.
// Content hash: 3334c83f45c4

import { z } from "zod";

/** A named participant slot declared by an automation's workflow. Embedded stages hand work to the agent the invoker names for the slot. */
export interface AutomationParticipantSlot {
  /** Workflow-authored explanation of the slot's role. `null` when the workflow declares none. */
  description?: string | null | undefined;
  /** The slot's name, as referenced by the workflow. Supply the chosen agent under the top-level `participants[name]` field when invoking. */
  name: string;
  /** Whether the workflow requires this slot to be filled for the run to complete its embedded stages. */
  required: boolean;
  /** The kind of principal the slot accepts. Currently always `"agent_user"` — the value supplied at invoke is an agent ID (`agi_...`). */
  type: string;
}

/** Runtime validator for {@link AutomationParticipantSlot}. */
export const automationParticipantSlotSchema = z.object({
  /** Workflow-authored explanation of the slot's role. `null` when the workflow declares none. */
  description: z.string().nullable().optional().describe("Workflow-authored explanation of the slot's role. `null` when the workflow declares none."),
  /** The slot's name, as referenced by the workflow. Supply the chosen agent under the top-level `participants[name]` field when invoking. */
  name: z.string().describe("The slot's name, as referenced by the workflow. Supply the chosen agent under the top-level `participants[name]` field when invoking."),
  /** Whether the workflow requires this slot to be filled for the run to complete its embedded stages. */
  required: z.boolean().describe("Whether the workflow requires this slot to be filled for the run to complete its embedded stages."),
  /** The kind of principal the slot accepts. Currently always `"agent_user"` — the value supplied at invoke is an agent ID (`agi_...`). */
  type: z.string().describe("The kind of principal the slot accepts. Currently always `\"agent_user\"` — the value supplied at invoke is an agent ID (`agi_...`)."),
}).describe("A named participant slot declared by an automation's workflow. Embedded stages hand work to the agent the invoker names for the slot.") satisfies z.ZodType<AutomationParticipantSlot>;

/** Locked payload and participant values supplied by the automation owner. */
export interface AutomationPrefills {
  /** Participant slot-to-agent mappings applied by the platform. Caller values at these slots must match exactly. */
  participants?: Record<string, unknown> | undefined;
  /** Partial invocation payload applied by the platform. A caller may omit these values, but supplying a different value at any locked path is rejected. */
  payload?: Record<string, unknown> | undefined;
}

/** Runtime validator for {@link AutomationPrefills}. */
export const automationPrefillsSchema = z.object({
  /** Participant slot-to-agent mappings applied by the platform. Caller values at these slots must match exactly. */
  participants: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("Participant slot-to-agent mappings applied by the platform. Caller values at these slots must match exactly."),
  /** Partial invocation payload applied by the platform. A caller may omit these values, but supplying a different value at any locked path is rejected. */
  payload: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("Partial invocation payload applied by the platform. A caller may omit these values, but supplying a different value at any locked path is rejected."),
}).describe("Locked payload and participant values supplied by the automation owner.") satisfies z.ZodType<AutomationPrefills>;

/** A single execution of an automation triggered by a platform event or direct invocation. Captures the run's status, input payload, and final result. */
export interface AutomationRun {
  /** ID of the app that owns this automation run (`dap_...`). */
  app: string;
  /** ID of the automation that was executed (`aut_...`). */
  automation: string;
  /** When the automation run was created (ISO 8601). */
  created_at?: string | undefined;
  /** ID of the platform event that triggered this run. `null` for directly invoked automations. */
  event_id?: string | undefined;
  /** Automation run ID (`atr_...`). */
  id: string;
  /** Invoke-time map of symbolic participant references to agent IDs. `null` when no participants were supplied. */
  participants?: Record<string, unknown> | undefined;
  /** The input event payload that triggered this run. Structure varies by automation type. Defaults to an empty object if no payload was provided. */
  payload?: Record<string, unknown> | undefined;
  /** The output produced after the automation finished executing. Contains workflow-defined keys alongside any returned output. `null` if the run has not yet completed. */
  result?: Record<string, unknown> | undefined;
  /** Current execution status of the run. One of `"pending"` (queued, not yet started), `"running"` (actively executing), `"completed"` (finished successfully), `"failed"` (finished with an error), or `"cancelled"` (stopped before completion). */
  status: string;
  /** ID of the team that owns this run (`tea_...`). `null` if the run is owned by a user rather than a team. */
  team?: string | undefined;
  /** When the automation run record was last updated (ISO 8601). */
  updated_at?: string | undefined;
  /** ID of the user that owns this run (`usr_...`). `null` if the run is owned by a team rather than a user. */
  user?: string | undefined;
}

/** Runtime validator for {@link AutomationRun}. */
export const automationRunSchema = z.object({
  /** ID of the app that owns this automation run (`dap_...`). */
  app: z.string().describe("ID of the app that owns this automation run (`dap_...`)."),
  /** ID of the automation that was executed (`aut_...`). */
  automation: z.string().describe("ID of the automation that was executed (`aut_...`)."),
  /** When the automation run was created (ISO 8601). */
  created_at: z.string().optional().describe("When the automation run was created (ISO 8601)."),
  /** ID of the platform event that triggered this run. `null` for directly invoked automations. */
  event_id: z.string().optional().describe("ID of the platform event that triggered this run. `null` for directly invoked automations."),
  /** Automation run ID (`atr_...`). */
  id: z.string().describe("Automation run ID (`atr_...`)."),
  /** Invoke-time map of symbolic participant references to agent IDs. `null` when no participants were supplied. */
  participants: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("Invoke-time map of symbolic participant references to agent IDs. `null` when no participants were supplied."),
  /** The input event payload that triggered this run. Structure varies by automation type. Defaults to an empty object if no payload was provided. */
  payload: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("The input event payload that triggered this run. Structure varies by automation type. Defaults to an empty object if no payload was provided."),
  /** The output produced after the automation finished executing. Contains workflow-defined keys alongside any returned output. `null` if the run has not yet completed. */
  result: z.record(z.custom<{} | null>((value) => value !== undefined)).optional().describe("The output produced after the automation finished executing. Contains workflow-defined keys alongside any returned output. `null` if the run has not yet completed."),
  /** Current execution status of the run. One of `"pending"` (queued, not yet started), `"running"` (actively executing), `"completed"` (finished successfully), `"failed"` (finished with an error), or `"cancelled"` (stopped before completion). */
  status: z.string().describe("Current execution status of the run. One of `\"pending\"` (queued, not yet started), `\"running\"` (actively executing), `\"completed\"` (finished successfully), `\"failed\"` (finished with an error), or `\"cancelled\"` (stopped before completion)."),
  /** ID of the team that owns this run (`tea_...`). `null` if the run is owned by a user rather than a team. */
  team: z.string().optional().describe("ID of the team that owns this run (`tea_...`). `null` if the run is owned by a user rather than a team."),
  /** When the automation run record was last updated (ISO 8601). */
  updated_at: z.string().optional().describe("When the automation run record was last updated (ISO 8601)."),
  /** ID of the user that owns this run (`usr_...`). `null` if the run is owned by a team rather than a user. */
  user: z.string().optional().describe("ID of the user that owns this run (`usr_...`). `null` if the run is owned by a team rather than a user."),
}).describe("A single execution of an automation triggered by a platform event or direct invocation. Captures the run's status, input payload, and final result.") satisfies z.ZodType<AutomationRun>;
