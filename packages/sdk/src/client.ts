// Copyright (c) 2026 ArchAstro Inc. Licensed under the MIT License.
// This file is auto-generated by @archastro/sdk-generator. Do not edit.
// Content hash: efb11a7ece74

import { forApp } from "./app-session.js";
import { AuthClient } from "./auth.js";
import { HttpClient } from "./runtime/http-client.js";
import { V1 } from "./v1.js";
import type { AppPlatformClient, ForAppOptions } from "./app-session.js";
import type { HttpClientConfig } from "./runtime/http-client.js";
import type { ActivityFeedResource } from "./v1/resources/activity_feed.js";
import type { AgentComputerResource } from "./v1/resources/agent_computers.js";
import type { AgentEnvVarResource } from "./v1/resources/agent_env_vars.js";
import type { AgentHealthActionResource } from "./v1/resources/agent_health_actions.js";
import type { AgentInstallationResource } from "./v1/resources/agent_installations.js";
import type { AgentRoutineRunResource } from "./v1/resources/agent_routine_runs.js";
import type { AgentRoutineResource } from "./v1/resources/agent_routines.js";
import type { AgentSessionResource } from "./v1/resources/agent_sessions.js";
import type { AgentSkillResource } from "./v1/resources/agent_skills.js";
import type { AgentToolResource } from "./v1/resources/agent_tools.js";
import type { AgentResource } from "./v1/resources/agents.js";
import type { AiResource } from "./v1/resources/ai.js";
import type { ArtifactResource } from "./v1/resources/artifacts.js";
import type { AutomationRunResource } from "./v1/resources/automation_runs.js";
import type { AutomationResource } from "./v1/resources/automations.js";
import type { BugReportResource } from "./v1/resources/bug_reports.js";
import type { ConfigResource } from "./v1/resources/config.js";
import type { CustomObjectResource } from "./v1/resources/custom_objects.js";
import type { EventSubscriptionDeliveryResource } from "./v1/resources/event_subscription_deliveries.js";
import type { EventSubscriptionResource } from "./v1/resources/event_subscriptions.js";
import type { ExternalObjectCapabilityResource } from "./v1/resources/external_object_capabilities.js";
import type { ExternalObjectResource } from "./v1/resources/external_objects.js";
import type { ExtractionResource } from "./v1/resources/extractions.js";
import type { FileResource } from "./v1/resources/files.js";
import type { InstallationSourceResource } from "./v1/resources/installation_sources.js";
import type { InviteResource } from "./v1/resources/invites.js";
import type { KnowledgeDocumentResource } from "./v1/resources/knowledge_documents.js";
import type { KnowledgeIngestionResource } from "./v1/resources/knowledge_ingestions.js";
import type { KnowledgeSourceResource } from "./v1/resources/knowledge_sources.js";
import type { KvResource } from "./v1/resources/kv.js";
import type { NotificationPreferenceResource } from "./v1/resources/notification_preferences.js";
import type { NotificationResource } from "./v1/resources/notifications.js";
import type { OauthResource } from "./v1/resources/oauth.js";
import type { OrgResource } from "./v1/resources/orgs.js";
import type { PrivateServiceDefinitionResource } from "./v1/resources/private_service_definitions.js";
import type { PrivateServiceEnrollmentResource } from "./v1/resources/private_service_enrollments.js";
import type { PrivateServiceResource } from "./v1/resources/private_services.js";
import type { SandboxResource } from "./v1/resources/sandboxes.js";
import type { ScriptResource } from "./v1/resources/scripts.js";
import type { SlackChannelBindingResource } from "./v1/resources/slack_channel_bindings.js";
import type { SolutionCategoryResource } from "./v1/resources/solution_categories.js";
import type { SolutionInstanceResource } from "./v1/resources/solution_instances.js";
import type { SolutionTagResource } from "./v1/resources/solution_tags.js";
import type { SolutionResource } from "./v1/resources/solutions.js";
import type { SshKeyResource } from "./v1/resources/ssh_keys.js";
import type { StatuResource } from "./v1/resources/status.js";
import type { TaskResource } from "./v1/resources/tasks.js";
import type { TeamMembershipResource } from "./v1/resources/team_memberships.js";
import type { TeamResource } from "./v1/resources/teams.js";
import type { ThreadMessageResource } from "./v1/resources/thread_messages.js";
import type { ThreadResource } from "./v1/resources/threads.js";
import type { TrajectoryResource } from "./v1/resources/trajectories.js";
import type { UserResource } from "./v1/resources/users.js";
import type { WorkItemResource } from "./v1/resources/work_items.js";
import type { WorkflowResource } from "./v1/resources/workflows.js";

export type PlatformClientConstructor<TClient extends PlatformClient = PlatformClient> =
  new (...args: any[]) => TClient;

export type PlatformClientExtension<
  TBase extends PlatformClientConstructor,
  TExtended extends PlatformClientConstructor,
> = (Base: TBase) => TExtended;

export interface PlatformClientConfig {
  baseUrl?: string;
  accessToken?: string;
  getAccessToken?: () => string | undefined;
  onRefreshToken?: () => Promise<string>;
  pathPrefix?: string;
  defaultHeaders?: Record<string, string>;
  /** Fetch credential policy for same-origin session gateways. */
  credentials?: RequestCredentials;
}

export interface PlatformClientClass<TClient extends PlatformClient = PlatformClient> extends PlatformClientConstructor<TClient> {
  extend<TExtended extends PlatformClientConstructor>(
    extension: PlatformClientExtension<this, TExtended>,
  ): PlatformClientClass<InstanceType<TExtended>>;
  withSecretKey(key: string, baseUrl?: string): TClient;
  withToken(apiKey: string, accessToken: string, baseUrl?: string): TClient;
  withCredentials(apiKey: string, email: string, password: string, baseUrl?: string): Promise<TClient>;
  forApp(options: ForAppOptions): AppPlatformClient<TClient>;
}

export class PlatformClient {
  readonly http: HttpClient;
  readonly auth: AuthClient;
  readonly v1: V1;
  readonly activity_feed: ActivityFeedResource;
  readonly agent_computers: AgentComputerResource;
  readonly agent_env_vars: AgentEnvVarResource;
  readonly agent_health_actions: AgentHealthActionResource;
  readonly agent_installations: AgentInstallationResource;
  readonly agent_routine_runs: AgentRoutineRunResource;
  readonly agent_routines: AgentRoutineResource;
  readonly agent_sessions: AgentSessionResource;
  readonly agent_skills: AgentSkillResource;
  readonly agent_tools: AgentToolResource;
  readonly agents: AgentResource;
  readonly artifacts: ArtifactResource;
  readonly automation_runs: AutomationRunResource;
  readonly automations: AutomationResource;
  readonly bug_reports: BugReportResource;
  readonly config: ConfigResource;
  readonly custom_objects: CustomObjectResource;
  readonly event_subscription_deliveries: EventSubscriptionDeliveryResource;
  readonly event_subscriptions: EventSubscriptionResource;
  readonly external_object_capabilities: ExternalObjectCapabilityResource;
  readonly external_objects: ExternalObjectResource;
  readonly extractions: ExtractionResource;
  readonly files: FileResource;
  readonly installation_sources: InstallationSourceResource;
  readonly invites: InviteResource;
  readonly knowledge_documents: KnowledgeDocumentResource;
  readonly knowledge_ingestions: KnowledgeIngestionResource;
  readonly knowledge_sources: KnowledgeSourceResource;
  readonly kv: KvResource;
  readonly notification_preferences: NotificationPreferenceResource;
  readonly notifications: NotificationResource;
  readonly orgs: OrgResource;
  readonly private_service_definitions: PrivateServiceDefinitionResource;
  readonly private_service_enrollments: PrivateServiceEnrollmentResource;
  readonly private_services: PrivateServiceResource;
  readonly sandboxes: SandboxResource;
  readonly scripts: ScriptResource;
  readonly slack_channel_bindings: SlackChannelBindingResource;
  readonly solution_categories: SolutionCategoryResource;
  readonly solution_instances: SolutionInstanceResource;
  readonly solution_tags: SolutionTagResource;
  readonly solutions: SolutionResource;
  readonly ssh_keys: SshKeyResource;
  readonly status: StatuResource;
  readonly tasks: TaskResource;
  readonly team_memberships: TeamMembershipResource;
  readonly teams: TeamResource;
  readonly thread_messages: ThreadMessageResource;
  readonly threads: ThreadResource;
  readonly trajectories: TrajectoryResource;
  readonly users: UserResource;
  readonly work_items: WorkItemResource;
  readonly workflows: WorkflowResource;
  readonly ai: AiResource;
  readonly oauth: OauthResource;

  constructor(config: PlatformClientConfig = {}) {
    const httpConfig: HttpClientConfig = {
      baseUrl: config.baseUrl ?? "https://platform.archastro.ai",
      accessToken: config.accessToken,
      getAccessToken: config.getAccessToken,
      onRefreshToken: config.onRefreshToken,
      pathPrefix: config.pathPrefix,
      defaultHeaders: config.defaultHeaders,
      credentials: config.credentials,
    };

    this.http = new HttpClient(httpConfig);
    this.auth = new AuthClient(this.http);
    this.v1 = new V1(this.http);
    this.activity_feed = this.v1.activity_feed;
    this.agent_computers = this.v1.agent_computers;
    this.agent_env_vars = this.v1.agent_env_vars;
    this.agent_health_actions = this.v1.agent_health_actions;
    this.agent_installations = this.v1.agent_installations;
    this.agent_routine_runs = this.v1.agent_routine_runs;
    this.agent_routines = this.v1.agent_routines;
    this.agent_sessions = this.v1.agent_sessions;
    this.agent_skills = this.v1.agent_skills;
    this.agent_tools = this.v1.agent_tools;
    this.agents = this.v1.agents;
    this.artifacts = this.v1.artifacts;
    this.automation_runs = this.v1.automation_runs;
    this.automations = this.v1.automations;
    this.bug_reports = this.v1.bug_reports;
    this.config = this.v1.config;
    this.custom_objects = this.v1.custom_objects;
    this.event_subscription_deliveries = this.v1.event_subscription_deliveries;
    this.event_subscriptions = this.v1.event_subscriptions;
    this.external_object_capabilities = this.v1.external_object_capabilities;
    this.external_objects = this.v1.external_objects;
    this.extractions = this.v1.extractions;
    this.files = this.v1.files;
    this.installation_sources = this.v1.installation_sources;
    this.invites = this.v1.invites;
    this.knowledge_documents = this.v1.knowledge_documents;
    this.knowledge_ingestions = this.v1.knowledge_ingestions;
    this.knowledge_sources = this.v1.knowledge_sources;
    this.kv = this.v1.kv;
    this.notification_preferences = this.v1.notification_preferences;
    this.notifications = this.v1.notifications;
    this.orgs = this.v1.orgs;
    this.private_service_definitions = this.v1.private_service_definitions;
    this.private_service_enrollments = this.v1.private_service_enrollments;
    this.private_services = this.v1.private_services;
    this.sandboxes = this.v1.sandboxes;
    this.scripts = this.v1.scripts;
    this.slack_channel_bindings = this.v1.slack_channel_bindings;
    this.solution_categories = this.v1.solution_categories;
    this.solution_instances = this.v1.solution_instances;
    this.solution_tags = this.v1.solution_tags;
    this.solutions = this.v1.solutions;
    this.ssh_keys = this.v1.ssh_keys;
    this.status = this.v1.status;
    this.tasks = this.v1.tasks;
    this.team_memberships = this.v1.team_memberships;
    this.teams = this.v1.teams;
    this.thread_messages = this.v1.thread_messages;
    this.threads = this.v1.threads;
    this.trajectories = this.v1.trajectories;
    this.users = this.v1.users;
    this.work_items = this.v1.work_items;
    this.workflows = this.v1.workflows;
    this.ai = this.v1.ai;
    this.oauth = this.v1.oauth;
  }

  /** Return a subclass augmented by a hand-maintained class-expression mixin. */
  static extend<TBase extends PlatformClientConstructor, TExtended extends PlatformClientConstructor>(
  this: TBase,
  extension: PlatformClientExtension<TBase, TExtended>,
): PlatformClientClass<InstanceType<TExtended>> {
    return extension(this) as unknown as PlatformClientClass<InstanceType<TExtended>>;
  }


  #refreshToken?: string;

  get refreshToken(): string | undefined {
    return this.#refreshToken;
  }

  setAccessToken(token: string) {
    this.http.setAccessToken(token);
  }

  setRefreshToken(token: string) {
    this.#refreshToken = token;
  }

  // ─── Factory constructors (generated from auth schemes) ───

  /** Secret API key — full admin access, no user JWT required */
  static withSecretKey<TClient extends PlatformClient>(this: PlatformClientConstructor<TClient>, key: string, baseUrl?: string): TClient {
    return new this({
      baseUrl,
      defaultHeaders: { "x-archastro-api-key": key },
    });
  }

  /** Create a client with a publishable key and pre-existing access token. */
  static withToken<TClient extends PlatformClient>(this: PlatformClientConstructor<TClient>, apiKey: string, accessToken: string, baseUrl?: string): TClient {
    return new this({
      baseUrl,
      accessToken,
      defaultHeaders: { "x-archastro-api-key": apiKey },
    });
  }

  /** Create a client by logging in with email/password */
  static async withCredentials<TClient extends PlatformClient>(this: PlatformClientConstructor<TClient>, apiKey: string, email: string, password: string, baseUrl?: string): Promise<TClient> {
    const client = new this({
      baseUrl,
      defaultHeaders: { "x-archastro-api-key": apiKey },
    });
    const tokens = await client.auth.login(email, password);
    if (!tokens.accessToken) throw new Error("Login did not return an access token");
    client.setAccessToken(tokens.accessToken);
    if (tokens.refreshToken) client.setRefreshToken(tokens.refreshToken);
    const refreshHttp = new HttpClient({
      baseUrl: baseUrl ?? "https://platform.archastro.ai",
      defaultHeaders: { "x-archastro-api-key": apiKey },
      refreshOnly: true,
    });
    const refreshAuth = new AuthClient(refreshHttp);
    client.http.setRefreshHandler(async () => {
      const rt = client.refreshToken;
      if (!rt) throw new Error("No refresh token available");
      const refreshed = await refreshAuth.refresh(rt);
      if (!refreshed.accessToken) throw new Error("Refresh did not return an access token");
      client.setAccessToken(refreshed.accessToken);
      if (refreshed.refreshToken) client.setRefreshToken(refreshed.refreshToken);
      return refreshed.accessToken;
    });
    return client;
  }

  /** Client-side app: publishable key + durable session storage. Wires passwordless OTP, restore/signIn/signOut, 401 auto-refresh, and createSocket(). */
  static forApp<TClient extends PlatformClient>(this: PlatformClientConstructor<TClient>, options: ForAppOptions): AppPlatformClient<TClient> {
    return forApp(options, this);
  }
}

export type { AppPlatformClient, ForAppOptions, SessionStorage, AppSession } from "./app-session.js";
