...
export class ConfigResource {
   ...
   * @param input.data_encoding - Encoding of `raw_content`. Omit or set `"raw"` for literal content; set `"base64"` when sending binary content such as images or PDFs in JSON.
   ...
   * @param input.data - Optional metadata used by kind-specific validation. File and Image configs require `data.name` when validating direct binary content.
   * @param input.data_encoding - Encoding of `raw_content`. Omit or set `"raw"` for literal content; set `"base64"` when sending binary content such as images or PDFs in JSON.
   ...
   * @param input.system - Set to true to validate as a system-owned config. Mutually exclusive with `team`, `user`, and `agent`.
   ...
   */
  async validate(input: { agent?: string | undefined; data?: Record<string, unknown> | undefined; data_encoding?: string | undefined; kind: string; mime_type: string; raw_content: string; system?: boolean | undefined; team?: string | undefined; user?: string | undefined }): Promise<ValidationResult> {
