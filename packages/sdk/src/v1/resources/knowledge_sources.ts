...
export class KnowledgeSourceResource {
   ...
   * @param params.team - Exact team IDs whose sources to include.
   * @param params.thread - Exact backing thread IDs whose sources to include.
   ...
   */
  async ingest(source: string, input: { content?: { content_type?: string | undefined; data: string; data_encoding?: string | undefined; filename?: string | undefined } | undefined; dedupe?: boolean | undefined; file?: string | undefined; metadata?: Record<string, unknown> | undefined; pull?: boolean | undefined; title?: string | undefined }): Promise<ContextIngestion> {
    return this.http.request(`/api/v1/knowledge_sources/${source}/ingest`, { method: "POST", body: input });
  }

  /**
   * Search within a knowledge source
   * Executes a search query against the indexed documents in a single knowledge source and
   ...
   * @param source - Knowledge source ID (`ksrc_...`) to search within.
   * @param input - Request body.
   * @param input.max_results - Maximum number of results to return. The platform applies its own upper bound; omit to use the default.
   ...
   * @param input.mode - Search algorithm to use. One of `"hybrid"` (default, combines vector and full-text), `"vector"` (semantic similarity only), or `"fulltext"` (keyword matching only).
   * @param input.query - Natural language or keyword query string to search for.
   * @param input.recency_days - When set, limits results to documents ingested within the last N days. Omit to search across all documents regardless of age.
   * @returns Successful response
   */
  async search(source: string, input: { max_results?: number | undefined; min_similarity?: number | undefined; mode?: string | undefined; query: string; recency_days?: number | undefined }): Promise<{ data: { content?: string | undefined; content_type?: string | undefined; created_at?: string | undefined; id: string; kind: "chunk"; metadata?: Record<string, unknown> | undefined; raw_content?: Record<string, unknown> | undefined; type?: string | undefined }[] }> {
    return this.http.request(`/api/v1/knowledge_sources/${source}/search`, { method: "POST", body: input });
  }
}
