...
export class SlackChannelBindingResource {
  ...
  /**
   * Assign a Slack channel's resident agent
   * Makes the given agent the channel's sole resident (one resident per
   ...
   * @param input - Request body.
   * @param input.agent_user_id - Agent user ID to assign as the channel's sole resident.
   * @param input.channel_id - Slack channel ID whose resident is being assigned (e.g. `C01234ABCDE`). A bare internal binding is created when the channel has none.
   * @param input.slack_team_id - Slack workspace team ID that the channel belongs to (e.g. `T01234ABCDE`). Identifies which Slack integration to use.
   * @returns The binding after assignment, with the attached agent list.
   */
  async assign(input: { agent_user_id: string; channel_id: string; slack_team_id: string }): Promise<SlackChannelBinding> {
    return this.http.request(`/api/v1/slack_channel_bindings/assign`, { method: "POST", body: input });
  }
