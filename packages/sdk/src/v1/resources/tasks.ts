...
// Content hash: d9aba586a396

import { HttpClient } from "../../runtime/http-client.js";
import type { Task, TaskComment, TaskExternalLink, TaskSessionLease, TaskSessionLeaseSummary } from "../../types/tasks.js";
...
export class CommentResource {
   ...
   * @returns Successful response
   */
  async list(task: string, params?: { team?: string; user?: string; agent?: string; org?: string; limit?: number; afterCursor?: string }): Promise<{ after_cursor?: string | undefined; before_cursor?: string | undefined; data: { author_actor?: { alias?: string | null | undefined; id?: string | null | undefined; name?: string | null | undefined; profile_picture?: { file?: string | null | undefined; height?: number | null | undefined; media?: string | null | undefined; mime_type?: string | null | undefined; refresh_url?: string | null | undefined; url?: string | null | undefined; width?: number | null | undefined } | null | undefined } | undefined; author_agent?: string | undefined; author_user?: string | undefined; body: string; created_at?: string | undefined; id: string; org?: string | undefined; sandbox?: string | undefined; task?: string | undefined; team?: string | undefined; updated_at?: string | undefined }[]; has_more: boolean }> {
    const query: Record<string, string | number | boolean | Array<string | number | boolean> | undefined> = {};
    if (params?.team !== undefined) {
      query["team"] = params?.team;
    }
...
export class LeaseResource {
   ...
   */
  async list(task: string): Promise<TaskSessionLeaseSummary | null> {
   ...
   * @param task - Task ID (`tsk_...`).
   * @param input - Request body.
   * @param input.force - Take the lease from a live but dead holder instead of returning a conflict. Releases the prior session's lease and claims the new one atomically. Always scoped to the task's assigned owner, same as an ordinary claim.
   ...
   * @param input.require_ready - Conservatively reject the claim when the current task projection has unfinished blockers.
   ...
   */
  async create(task: string, input: { force?: boolean | undefined; harness: string; lease_duration_seconds?: number | undefined; lease_id: string; require_ready?: boolean | undefined; session_id: string; session_name: string }): Promise<TaskSessionLease> {
...
export class LinkResource {
   ...
   */
  async remove(task: string): Promise<void> {
    await this.http.request(`/api/v1/tasks/${task}/links`, { method: "DELETE" });
  }

  /**
   * List a task's external links
   * Returns indexed link identities ordered by external_scope, object_type, then object_id ascending. This does not read the legacy Task links map.
   * @param task - Task ID (`tsk_...`).
   * @param params - Query parameters.
   * @param params.team - Explicit owning team for privileged calls.
   * @param params.user - Explicit owning user for privileged calls.
   * @param params.agent - Explicit owning agent for privileged calls.
   * @param params.org - Explicit organization for privileged calls; must agree with owner.
   * @param params.limit - Maximum links to return. Capped at 100.
   * @param params.afterCursor - Opaque cursor returned by the previous page.
   * @returns Successful response
   */
  async list(task: string, params?: { team?: string; user?: string; agent?: string; org?: string; limit?: number; afterCursor?: string }): Promise<{ after_cursor?: string | undefined; before_cursor?: string | undefined; data: { external_scope: string; object_id: string; object_type: string }[]; has_more: boolean }> {
    const query: Record<string, string | number | boolean | Array<string | number | boolean> | undefined> = {};
    if (params?.team !== undefined) {
      query["team"] = params?.team;
    }
    ...
    return this.http.request(`/api/v1/tasks/${task}/links`, { query });
  }

  /**
   * Add an external link to a task
   * Adds an indexed identity without changing the referenced object's ownership or ACL.
   ...
   * @param task - Task ID (`tsk_...`).
   * @param input - Request body.
   * @param input.external_scope - External container ID.
   * @param input.object_id - External object ID.
   * @param input.object_type - External object type.
   * @returns The created external link, with its identity normalized.
   */
  async create(task: string, input: { external_scope: string; object_id: string; object_type: string }): Promise<TaskExternalLink> {
    return this.http.request(`/api/v1/tasks/${task}/links`, { method: "POST", body: input });
  }
}
...
export class TaskResource {
  ...
  constructor(private http: HttpClient) {
    ...
    this.links = new LinkResource(http);
  }
   ...
   * @param input.epic - Replacement grouping label. Pass null to clear it.
   * @param input.expected_version - Aggregate version returned by the latest task read. The update fails with `task_version_conflict` if the task changed first.
   ...
   * @param input.source_id - Replacement source object identity. Must be supplied with the other source fields.
   * @param input.source_scope - Replacement source container. Pass together with `source_type` and `source_id`, or pass all three as null to clear the source.
   * @param input.source_type - Replacement source object kind. Must be supplied with the other source fields.
   * @param input.status - Updated status: `open`, `in_progress`, `in_review`, or `done`.
